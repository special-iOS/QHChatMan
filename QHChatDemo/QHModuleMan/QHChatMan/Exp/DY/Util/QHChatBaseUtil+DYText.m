...
+ (NSAttributedString *)toUserLevelImage:(UIImage *)image {
    return [QHChatBaseUtil toImage:image size:CGSizeMake(0, 13) offBottom:-2];
}
...
+ (NSAttributedString *)toImage:(UIImage *)image size:(CGSize)size titleLabelBlock:(void(^)(UILabel *titleL))block {
    NSAttributedString *imageAttr = [self toImage:image size:size offBottom:-2 addContentBlock:^(UIImageView * _Nonnull imageV) {
